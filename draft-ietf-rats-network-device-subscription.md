...
The terms defined in {{!RFC9684}} are mapped with the Attestation Subscription Model (without a Broker) defined {{-rats-models}} for the specialized sequence diagram illustrated below.

The following terms defined in {{-rats-models}} are mapped:
* 'handle' is specialized by a 'nonce' generated by the Verifying Relying Party that is a nonce-value byte string from either the tpm12-challenge-response-attestation RPC or the tpm20-challenge-response-attestation RPC as specified in {{!RFC9684}}
* 'attEnvIDs' is specialized by 'TpmName' that is a TPM "name" text string selected from the 'tpms' Container as specified in {{!RFC9684}}
* 'claimsSelection' is specialized by an optional 'PcrSelection' that is a "pcr-index" from either the tpm12-challenge-response-attestation RPC or the tpm20-challenge-response-attestation RPC as specified in {{!RFC9684}}. If no PCR is selected, all PCR banks are returned.
* 'claims' is specialized by 'PcrQuotes' as specified in the "output" of either the tpm12-challenge-response-attestation RPC or the tpm20-challenge-response-attestation RPC as specified in {{!RFC9684}}. In contrast to event logs, there is no delta to a previous iteration of PCR Quotes during a subscription, but all new (selected) Quotes are conveyed as fresh Evidence.
* 'eventLogs' represents "system-event-logs" as specified in the "output" of the log-retrieval RPC as defined in {{!RFC9684}}.
