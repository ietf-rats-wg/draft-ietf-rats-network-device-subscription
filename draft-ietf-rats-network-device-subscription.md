...
Specifically, this document defines a YANG module that augments the YANG module for TPM-based Challenge-Response Remote Attestation (CHARRA), enabling subscription to RATS Conceptual Messages as defined in {{Section 1 of -cmw}} of the Evidence type and auxiliary Event Logs as part of that Evidence.
...
{{-rats-riv}} and {{-charra}} define the operational prerequisites and a YANG Model for acquiring Evidence from a network device containing at least one Trusted Platform Module (TPM 1.2, TPM 2.0, or equivalent hardware implementations providing the same protected capabilities {{TCG-Glossary}} as a TPM).
...
It is common to convey Conceptual Messages as defined in {{Section 1 of -cmw}} ad-hoc or periodically via requests.
...
* `claimsSelection` is substituted with `PcrSelection`, an optional "pcr-index" from either the tpm12-challenge-response-attestation RPC or the tpm20-challenge-response-attestation RPC as specified in {{-charra}}. If no Platform Configuration Record (PCR) is selected, all PCR banks are returned.
