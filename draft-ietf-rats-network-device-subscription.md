...
### Binding to the Streaming Remote Attestation Model

The following identifies how the abstract parameters of Streaming Remote Attestation without a Broker, as described in {{Section 7.3.1 of -rats-models}}, are represented by the CHARRA and YANG elements used by this specification. This binding does not redefine the terminology of either source specification.

The protocol bindings used by {{term-sequence}} are as follows:

* `handle` is represented by the CHARRA information element named `nonce`. For the initial Evidence, this element carries the fresh
  Verifier-generated nonce supplied with the subscription. For subsequent Evidence, the same information element carries the
  freshness handle applicable to Streaming Remote Attestation, as described in {{Section 6 of -rats-models}} and {{freshness-handles}}.

* `attEnvIDs` is represented by `TpmName`, a TPM "name" text string selected from the `tpms` Container, as specified in {{-charra}}
...
* If either the \<reset-counter\> or \<restart-counter\> has incremented, the existing subscription MUST be terminated. If the Verifier intends to continue receiving attestation Evidence, it MUST establish a new subscription.

* If a TPM Quote on any subscribed PCR has not been pushed to the Verifier for the Attester-defined heartbeat interval, a new TPM Quote notification containing that PCR MUST be sent to the Verifier.
...
This notification type documents when a subscribed PCR is extended within a single TPM cryptoprocessor.  It MUST be emitted no later than the \<marshalling-period\> after the PCR is first extended, except when the marshalling period is extended as described below.  (The reason for the marshalling is that it is quite possible that multiple extensions to the same PCR have been made in quick succession, and these should be reflected in the same notification.)  This notification MUST be emitted prior to a \<tpm12-attestation\> or \<tpm20-attestation\> notification which has included and signed the results of any specific PCR extension.  If PCR extending events occur during the generation of the \<tpm12-attestation\> or \<tpm20-attestation\> notification, the marshalling period MUST be extended so that a new \<pcr-extend\> is not sent until the corresponding notifications have been sent.
...
* every time at least one PCR has changed from a previous \<tpm20-attestation\>. In this case, the notification SHOULD be emitted within 10 seconds of the corresponding \<pcr-extend\> being sent. The 10-second interval is fixed by this specification and is not a configurable parameter.
